...
const Navbar = () => {
  ...
  const userMenuItems = [
    { path: '/profile', label: 'Profil', icon: <User className="h-4 w-4 mr-2" /> },
    { path: '/my-reservations', label: 'Rezervasyonlarım', icon: <Calendar className="h-4 w-4 mr-2" /> },
    { path: '/loyalty', label: 'Sadakat Programı', icon: <Crown className="h-4 w-4 mr-2" /> }
  ];
  
  const isActive = (path: string) => location.pathname === path;
  
  return (
    <nav className="bg-white border-b z-50 sticky top-0 shadow-sm">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between h-16">
          <div className="flex justify-between w-full">
            <div className="flex-shrink-0 flex items-center">
              <Link to="/" className="text-xl font-bold">
                Lovable<span className="text-primary">Resto</span>
              </Link>
            </div>
